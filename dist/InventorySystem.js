...
function updateStock(itemId, quantity) {
    var stock = inventory.filter(function (item) { return item.itemId === itemId; })[0];
    stock.details[1] = quantity;
    return "Stock updated for ".concat(stock.details[0], ", new quantity: ").concat(stock.details[1]);
}
